...
public:
	enum BufInfo {
		DUMB_BUFFERS = 0x1, GEM_BUFFERS = 0x2, BOTH_BUFFERS = DUMB_BUFFERS
				| GEM_BUFFERS
	} buf_info;
	...
	LoaderInfo(DriverVersion _driver_version = unknown, BufInfo _buf_info =
			BOTH_BUFFERS);
};
...
public:
	...
	static Loader * getInstance(bool force_restart = false,
			const LoaderInfo params = LoaderInfo());
	~Loader();
};
