...
Renderer::Loader::Loader(const LoaderInfo &_l_info) :
		next_connector_pair_ptr(0), loader_log("loader: ", std::cerr), l_info(
				_l_info) {
	loader_log() << "Loading...";
...
std::pair<drmModeConnector *, drmModeCrtc *> Renderer::Loader::getNextConnectorPair() {
	...
	while (next_connector_pair_ptr != drm_mode_res->count_connectors) {
		conn = drmModeGetConnector(fd,
				drm_mode_res->connectors[next_connector_pair_ptr++]);
	...
	return_result: drmModeFreeEncoder(enc);
...
Renderer::Loader* Renderer::Loader::getInstance(bool force_restart,
		const LoaderInfo params) {
...
Renderer::LoaderInfo::LoaderInfo(
		Renderer::LoaderInfo::DriverVersion _driver_version,
		Renderer::LoaderInfo::BufInfo _buf_info) :
		driver_version(_driver_version), buf_info(_buf_info) {
}

Renderer::Loader::~Loader() {
	loader_log() << "loader removed ... ";
}
